...
function Reposts() {
  ...
  const [myReposts, setmyReposts] = useState([
    {
      ...
      aImg: TrapstarM,
      aAlt: "Trapstar Bam and CD Mumba candid photo"
    },
    {
      aName: "Simplify",
  ...
  const addReposts = (reposts) => {
    const newReposts = [...myReposts, { reposts }];
    setmyReposts(newReposts);
  };
  const removeReposts = (id) => {
    const newReposts = [...myReposts];
    newReposts.splice(id, 1);
    setmyReposts(newReposts);
  };
  return (
    <section style={styles.container}>
      <h1>{pageTitle}</h1>
      <PostForm addReposts={addReposts} />
      {myReposts.map((myReposts, id) => (
        <RepostsList
          key={id}
          id={id}
          myReposts={myReposts}
          removeReposts={removeReposts}
        />
      ))}
    </section>
  );
